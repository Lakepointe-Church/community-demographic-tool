...
- **Spec v2 Phase 4** — New data sources (in progress):
  ...
  - **4.3** ✅ (code; awaiting first data load) — ZIP-level school enrollment via NCES CCD (Urban Institute API): `zip_school_enrollment` table, `scripts/import-school-enrollment.ts` (self-verifying), `/api/school-enrollment?zip=`. Site Scorer now **prefers ZIP-level enrollment CAGR over county TEA CAGR** (`enrollmentSource` field exposes which was used), fixing the suburban/urban district blending. Chose NCES CCD over TEA TAPR campus files (ships ZIP + enrollment in one source — no manual download or geocoding; [HUMAN] decision). **Run the importer from a normal network** (Urban API Cloudflare-blocks some datacenter IPs); table empty until then → scorer falls back to county. Federal source, ~1–2yr lag.
