...
| `/demographics` | ✅ | Per-ZIP: 8 stat cards + Dual-Earner HH + Long Commute cards, YFI/WFI indexes, race donut, education bars, age bars, household type donut, college scorecard. **Phase 4.2:** Leading Indicators panel includes a full-width horizontal bar list of top permit-issuing cities in the ZIP's county (Census BPS place-level, 2023–2025, with YoY% delta badges). **Phase 4.5:** Commute Corridors panel (LODES8) — net commute direction (compass + rotated arrow + concentration), resident workers, self-containment %, and top work-destination ZIPs (blue bar list with high-earner ▸% badges). **Phase 4.1 (scaffold):** Address Momentum panel (HUD USPS) — active residential addresses, trailing-4Q % change, quarterly sparkline; renders only once HUD data is loaded. **Phase 4.4:** Giving Capacity panel (IRS SOI) — avg charitable gift per giving return, charitable % of AGI, itemizer rate; renders only when the ZIP has SOI data. **Phase 4.7:** Home Value Trend panel (Zillow ZHVI) — typical home value, YoY % badge, trailing-year sparkline; renders only when the ZIP has ZHVI data. **Phase 4.6:** Migration Flows panel (IRS SOI county-to-county) — net/in/out households per year, avg AGI of in- and out-migrants, top origin counties bar list (households + per-county avg AGI badge); county-level (the ZIP's county), renders only when the ZIP's county has migration data. |
| `/compare` | ✅ | Multi-select ZIP comparison: combined stats, race donut, income chart, summary table |
| `/ses-classes` | ✅ | Overview/detail split (mirrors Employers): **ZIP dropdown** top-right (DFW Overview default). Overview = 4 stat cards + distribution bar chart + scatter (SES score vs income) + **top-10** sortable table with tier-filter tabs (capped at 10; CSV still exports all; ⚠ unreliable toggle). Selecting a ZIP swaps in a per-ZIP detail: class badge + score/100 + rank, **Score Composition** breakdown (income 50% / bachelor's 30% / home value 20% contributions via `sesComponents`), and an indicators grid (HHI, home value, %Bachelor's, %Mgmt/Prof, unemployment, trend) |
...
- **IRS SOI county-to-county migration (filing years 2022→2023)** — county-level migration flows from year-over-year tax-return address changes. `scripts/import-soi-migration.ts` streams the `countyinflow2223.csv` + `countyoutflow2223.csv` files from irs.gov/pub/irs-soi (no key), filters to the 23 DFW focus counties, keeps the IRS grand-total row + top-25 real counterpart counties per direction (dropping the 96/97/98 summary pseudo-rows, the 57–59 "Other flows" suppression aggregates, and the non-migrant self-row), and stores them in `county_migration`. Drives the Migration Flows panel on Demographics (net/in/out households, avg AGI of movers, top origin counties). Annual vintage (lags ~2–3yr). Context/narrative tier — never scored. Re-run annually after the ~Nov release (bump `2223`→next pair in the script).
...
External APIs (Census ACS, Census CBP, BLS, FRED)
  ...
  /api/migration?zip=          ← IRS SOI county migration (ZIP's county): inbound/outbound household totals + net + avg AGI of in/out-migrants + top origin/destination counties (reads county_migration; available:false until import-soi-migration.ts run)
        ↓
...
**`county_migration`** — IRS SOI county-to-county migration flows (Phase 4.6), loaded via `scripts/import-soi-migration.ts`
Columns: `direction` (TEXT — `in`|`out`), `fips` (TEXT — DFW focus county, 5-digit `48xxx`), `county` (TEXT — DFW county name), `year` (INT — destination year, 2023 for the 2022→2023 file pair), `kind` (TEXT — `total` = IRS grand-total pseudo-row | `county` = a real county pair), `other_fips` (TEXT — counterpart county FIPS; `96000` for the total row), `other_name` (TEXT), `other_state` (TEXT — state abbrev, null for total), `returns` (INT — households), `individuals` (INT), `agi` (BIGINT — $1000s), `updated_at`
PK: `(direction, fips, other_fips, year)`. Index on `(fips, direction, year)`. ~814 rows (23 counties × 2 directions × [1 total + ≤25 counterparts]). Context tier only.
...
- **IRS SOI county migration** — annually after the ~November release: `npx tsx scripts/import-soi-migration.ts` (auto-fetches inflow + outflow CSVs; bump `2223`→next filing-year pair + `YEAR` in the script when a newer vintage lands)
...
src/lib/scoring.test.ts                — Vitest suite: 25 hand-computed fixtures covering every scoring.ts formula incl. sesComponents, null-growth redistribution + threshold boundaries. Run `npm test`
...
scripts/import-soi-migration.ts        — IRS SOI county migration loader (Phase 4.6); streams countyinflow2223.csv + countyoutflow2223.csv from irs.gov (local cache data/countyin/outflow2223.csv optional), filters to DFW focus counties, keeps grand-total + top-25 real counterparts per direction, DELETE-by-year + reinsert county_migration. DRY_RUN=1 to preview. Re-run annually
src/app/api/migration/route.ts         — GET /api/migration?zip= — IRS SOI county migration for a ZIP's county: inbound/outbound/net households + avg AGI of in/out-migrants + top origin/destination counties from county_migration
...
- **Spec v2 Phase 3** ✅ — Executive decision layer: shareable scenario URLs (weights encoded in URL, Copy Link button), score percentile anchoring (top X% in sidebar + table), top-3 insights panel on Overview page (fit score + top drivers), distance-to-campus signal (✅ now wired — straight-line haversine to nearest existing campus, default off, see §Lakepointe Fit Score), Vercel Analytics (`@vercel/analytics/next` in root layout), preset weights (Balanced/Young Families/Underserved), campus highlights on scatter + table, decision log (`decision_log` table + `/api/decisions` + `/admin/decisions`)
  - **Spec v2 Phase 3.6** ✅ (boardroom UX, accessibility slice) — **Reload relabel** (coverage "↺ Reload" buttons + Site Scorer tooltip) and **tap-to-pin map popovers** (persistent click popup w/ close button) were already shipped. **WCAG AA contrast sweep:** lifted the two AA-failing dark-theme text colors across all 15 dark pages/components — footer `#5a6478` (~3.2:1) → `#7A8699` (~5.2:1), faint `#3d4a5c` (~2:1) → `#6E7C92` (~4.5:1) — and floored the smallest mono labels from 9px → 10px for the 55+ leadership audience; `theme.ts` tokens updated to match. The white-bg print one-pager was deliberately excluded (its greys are already high-contrast on white). **Boardroom/projection mode: skipped per [HUMAN]** (Jolie) — the print one-pager already covers the print/projection need; revisit if a multi-ZIP boardroom artifact is requested.
- **Spec v2 Phase 4** — New data sources (in progress):
  ...
  - **4.6** ✅ — IRS SOI county-to-county migration: `county_migration` table, `scripts/import-soi-migration.ts` (streams the `countyinflow2223.csv` + `countyoutflow2223.csv` files, filters to 23 DFW focus counties, keeps grand-total + top-25 real counterpart counties per direction), `/api/migration?zip=`, Migration Flows panel on Demographics (net/in/out households, avg AGI of movers, top origin counties). Filing years 2022→2023, ~814 rows. **Note:** the county files carry total AGI per county-pair, not AGI-stub bands (the spec's "(AGI bands)" overreaches the actual file) — "at what income" = avg AGI per migrating return. Context/narrative tier — never scored.
...
- **Spec v2 Phase 5** — Code health (ongoing, interleave):
  ...
  - **5.2** 🟡 — Shared component library: `src/lib/theme.ts` (tokens — `colors`/`fonts`/`rgbMap`/`toRgb`/`CARD_BG`/`cardGlow`/`cardGlowRgb`) + `src/components/ui/` (`StatCard` w/ `compact` variant = hex-`color`/colored-value family; `StatCardAccent` = named-`accent`/top-bar/white-value/`tooltip` family; `StatCardFlex` = flex-sized/`borderRadius:10`/raw-`rgb`/`unit` family; `Surface`; `SectionHeader` = small mono panel label+sub; `SectionTitle` = gold-left-border eyebrow+Bebas title). **Migrated (all 7 data pages):** community-needs, employers, ses-classes (Family-B `StatCard`+`Surface`+`SectionHeader`); Overview/`page.tsx`, compare, demographics (Family-A `StatCardAccent as StatCard` + `SectionTitle as SectionHeader`, alias-imported to keep call sites, all `CARD_SURFACE`-const **and** inline-gradient panels → `<Surface>`); religious (`StatCardFlex as StatCard`). All behavior-preserving (tsc + build clean, every page still prerenders static). religious's solid-`#13161f`/rounded/gold-tinted panels are a deliberately different surface treatment and stay inline (not the `CARD_BG` gradient `Surface`). **Remaining:** `<DataTable>` not extracted — per-page tables genuinely diverge (sort keys, color thresholds, sticky headers, CSV export), so a shared abstraction would be all config and no savings; left until a clean two-table case justifies it. Incremental, verify rendering after each.
  ...
  - **Governance reframe** (spec Appendix A): source throughput is gated by *tier*, not a flat monthly quota — context-tier sources add freely during build-out if they clear admission gates; scored signals ≤1/quarter behind tests + ~8-cap + no-double-count + [HUMAN]; ~1/month for everything post-launch. Build order: 5.1 (done) → 5.4 → 4.3 (scored, behind tests), context sources (4.6 ✅ done, 4.1-when-unblocked) interleaved.
