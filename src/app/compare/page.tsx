  ...
  return (
    <div style={{ background: '#13161f', border: '1px solid #1e2433', padding: '22px', position: 'relative', overflow: 'hidden' }}>
      ...
      {loading ? (
        ...
        <>
          <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '36px', lineHeight: 1, letterSpacing: '0.03em', color: '#F0F2F7' }}>{value}</div>
  ...
  return (
    ...
    >
      ...
      {[0, Math.round(maxPct / 2), Math.round(maxPct)].map(v => {
        ...
        return (
          <g key={v}>
            <text x={padL - 6} y={y + 4} textAnchor="end" fill="#6B7689" fontFamily="IBM Plex Mono" fontSize="9">{v}%</text>
        ...
        return (
          <g key={b.label}>
            <rect
              ...
              fill="#4EAEFF" opacity={0.75}
              style={{ transition: 'all 0.6s ease' }}
            />
            <rect x={x} y={chartH - barH} width={barW} height={2} fill="#4EAEFF" opacity={0.5} />
            {!loading && b.pct > 0 && (
              <text x={x + barW / 2} y={chartH - barH - 5} textAnchor="middle" fill="#F0F2F7" fontFamily="IBM Plex Mono" fontSize="9">
                {b.pct.toFixed(1)}%
              </text>
            )}
            <text x={x + barW / 2} y={chartH + 16} textAnchor="middle" fill="#9BA5B7" fontFamily="IBM Plex Mono" fontSize="8">
              {b.label}
            </text>
          </g>
        )
  ...
  return (
    <>
      <style>{`
        ...
        .zip-toggle .check {
          width: 14px; height: 14px; border: 1px solid #1e2433; border-radius: '2px';
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ padding: '36px 32px', maxWidth: '1440px', margin: '0 auto' }}>
          ...
          {/* ZIP Selection Grid */}
          <div className="fade-up-2" style={{ background: '#13161f', border: '1px solid #1e2433', padding: '20px', marginBottom: '28px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
              <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', letterSpacing: '0.12em', color: '#9BA5B7', textTransform: 'uppercase' as const }}>
                Select ZIP Codes
              </span>
              {selected.size > 0 && (
                <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '10px', color: '#2DD4BF', letterSpacing: '0.08em' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '6px' }}>
              {DFW_ZIPS.map(({ zip, label }) => (
                ...
                >
                  ...
                  <span>
                    <span style={{ color: selected.has(zip) ? '#2DD4BF' : '#9BA5B7', marginRight: '6px' }}>{zip}</span>
                    <span style={{ color: selected.has(zip) ? '#C8D0DC' : '#6B7689', fontSize: '10px' }}>{label}</span>
                  </span>
                </button>
              ))}
            </div>
          ...
          {/* Combined Stats */}
          {selected.size === 0 ? (
            <div style={{ background: '#13161f', border: '1px solid #1e2433', padding: '48px', textAlign: 'center' }}>
              <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '12px', color: '#6B7689', letterSpacing: '0.08em' }}>
                ...
                {loading ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {Array.from({ length: 3 }).map((_, i) => (
                      <div key={i} style={{ height: '32px', background: '#1e2433', borderRadius: '2px', animation: 'pulse 1.5s ease-in-out infinite' }} />
                    ))}
                  </div>
                ) : (
                  <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr repeat(4, 120px)', gap: '0' }}>
                    {['ZIP', 'Name', 'Population', 'Median HHI', 'Growth', 'SES'].map(h => (
                      <div key={h} style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '9px', color: '#6B7689', letterSpacing: '0.1em', textTransform: 'uppercase' as const, padding: '8px 12px 8px 0', borderBottom: '1px solid #1e2433' }}>{h}</div>
                    ))}
                    {zipData.map(d => [
                      d.zip,
                      ...
                      <div key={j} style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '11px', color: j === 0 ? '#2DD4BF' : '#F0F2F7', padding: '10px 12px 10px 0', borderBottom: '1px solid #1a1f2e', letterSpacing: j === 4 ? '0' : '0.02em' }}>
                        {cell}
                      </div>
                    )))}
                  </div>
